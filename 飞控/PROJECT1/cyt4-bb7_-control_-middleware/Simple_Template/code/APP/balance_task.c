...
#define DefalutHeight 0.15f  //启动时的默认高度
//对四轴的控制量
...
// 油门控制
static short FlyControl_throttle= 0;
...
// 光流数据回调函数
void getOpticalFlowResult_Callback(void)
{
  
        pmw3901_get_motion();
	speedY = -pmw3901_delta_y ;// 200.0f; // 速度(m/s)
	speedX = -pmw3901_delta_x ;// 200.0f; // 速度(m/s)
...
ATTITUDE_DATA_t* p_attitudeVal = NULL;
...
void balance_task(void* param)
{
	...
	//任务频率、时间调试变量
	pRtosDebugInterface_t debug = &UserSoft_TimerDebug;
...
//        pFLOWInterface_t flow = &UserPMW3901;
	...
	//ADC驱动
	pBATTERYInterface_t adc1 = &UserBATTERY;
	...
	ATTITUDE_DATA_t AttitudeVal = { 0 };
        p_attitudeVal= &AttitudeVal;
        
//        FLOW_DATE_t flow_i = {0}; 
...
//	userset_roll = g_userparam_rollzero;
	...
	while(1)
	{
                ...
                if (RATE_DO_EXECUTE(RATE_50_HZ, tick))
		{
                    ...
                    g_readonly_distance= dl1b_distance_m;
                    use_distance =g_readonly_distance - zero_distance;
		...
		if (RATE_DO_EXECUTE(RATE_250_HZ, tick) &&  (uxBits & IMU_axilValZeroDone_Event))
...
//                flow->UpdateFlow(&AttitudeVal,use_distance,0.005,&flow_i);
...
//                if( controlY <-limitPos ) controlY = -limitPos;
		...
		{
                    //电压获取
                    g_robotVOL = (float)adc1->getValue()/4095.0f*3.3f*6.0f;
		}
                
//                getOpticalFlowResult_Callback();
//                pmw3901_get_motion();
    ...
    //					xEventGroupSetBits(g_xEventFlyAction,StartFly_Event);
                ...
                if (RATE_DO_EXECUTE(RATE_50_HZ, tick))// 读取控制信息
                {
                    ...
                    if( pdPASS == xQueueReceive(g_xQueueFlyControl,&controlVal,0) )
                    {
                            ...
                            controlVal.setup       == 0?xEventGroupClearBits(g_xEventFlyAction,StartFly_Event):xEventGroupSetBits(g_xEventFlyAction,StartFly_Event);
                            controlVal.height_setup== 0?xEventGroupClearBits(g_xEventFlyAction,TestSetuphightControl):xEventGroupSetBits(g_xEventFlyAction,TestSetuphightControl);
                            
                                                        //Z轴控制量为累计值
                            FlyControl_yaw -= angle_to_rad(controlVal.gyroz);
                            FlyControl_roll = angle_to_rad(controlVal.roll);
                            FlyControl_pitch= angle_to_rad(controlVal.pitch);
                           
                            FlyControl_throttle= controlVal.throttle;
                            FlyControl_height = controlVal.height;
                            
                            userset_pitch= angle_to_rad(controlVal.set_pitch);
                            userset_roll = angle_to_rad(controlVal.set_roll);
...
//		
//		/* 高度限幅处理 */
//                FlyControl_height= 0.15;
                
                
		
		
                float a_motion_x,a_motion_y,a_motion_xlast,a_motion_ylast,x_dot_last,y_dot_last;
                float targetDotX=0,targetDotY=0,last_controlX,last_controlY;
...
//                        speedY = speedY - fmaxf(fminf(gy_filtered,2.0f),-2.0f); // 旋转补偿 + 尺度缩放
//                        speedX = use_distance * (speedX + fmaxf(fminf(gx_filtered,2.0f),-2.0f)); // 旋转补偿 + 尺度缩放
      ...
      //                        if (use_distance > 0.1f) 
      //                        { // 高度低于0.3m时数据不可靠，暂不补偿
                                  speedX = flow_o.loc_xs;// m/s
                                  speedY = flow_o.loc_ys;// m/s
      //                        } else {
      //                            // 低空时，为避免高度误差放大，直接置零
      //                            speedX = 0;// m/s
      //                            speedY = 0;// m/s
      ...
      ////			
        ...
        ////			// 去除自身加速度，纯运动加速度  方向，东北天  AB侧为x轴正，BC侧为y轴正
      ...
      ////			// 坐标变换(机体坐标系 --> 世界坐标系)
        ...
        ////			
        ////			// 互补滤波
                                x_dot = (x_dot + a_motion_x * 0.005f) ;//+  0.15f * speedX;// 单位：m/s
                                y_dot = (y_dot + a_motion_y * 0.005f) ;//+  0.15f * speedY;// 单位：m/s
                                
                              

                              
                              
      //                        if(x_dot<0.005f && x_dot>-0.005f)x_dot = 0;
      //                        if(y_dot<0.005f && y_dot>-0.005f)y_dot = 0;
                              
      //                        x_dot += (x_dot - x_dot_last) *0.15;
      //                        y_dot += (y_dot - y_dot_last) *0.15;
      //                        x_dot_last = x_dot;y_dot_last = y_dot;
      ...
      ////			//位置PD计算
                              const float limitPos = 0.35f;
                               controlX = 1.5f * (targetPosX - posX) ;//+ 4.0f  * (targetDotX-x_dot);//前后
                               controlY = 1.5f * (targetPosY - posY) ;//+ 4.0f  * (targetDotY-y_dot);//左右
      //                        controlX += (controlX - last_controlX) *0.2;
      //                        controlY += (controlY - last_controlY) *0.2;
      //                        last_controlX = controlX;last_controlY = controlY;
...
//			}
    ...
    //			
                        ...
                        if (RATE_DO_EXECUTE(RATE_50_HZ, tick) && (uxBits & TestSetuphightControl))  // 开启高度控制位
                        {
                             //起飞时执行高度平滑控制
...
//                            }
      ...
      //			uint16_t base_throttle = weight_to_throttle((145.0f+0+FlyControl_unuseHeight*100.0f),use_distance); //基础油门值
                              uint16_t base_throttle = FlyControl_throttle;
		...
		if (RATE_DO_EXECUTE(RATE_200_HZ, tick))
                {
                      ...
                      appshow.pitch = AttitudeVal.pitch*57.2958;
                      appshow.roll  = AttitudeVal.roll*57.2958;
                      appshow.yaw   = AttitudeVal.yaw*57.2958;
                      ...
                      appshow.c_pitch = FlyControl_pitch*57.2958; // 转换成角度
                      appshow.c_roll = FlyControl_roll*57.2958;
                      appshow.c_yaw = FlyControl_gyroz*57.2958;
                      
                      appshow.c_height = FlyControl_height;
                      appshow.zero_roll = userset_roll;
                      appshow.zero_pitch = userset_pitch;
                      
      //		appshow.posx = flow_o.loc_x;
      //		appshow.posy = flow_o.loc_y;
                      ...
                      appshow.speedx = pmw3901_delta_x;
                      appshow.speedy = pmw3901_delta_y;
                      
      //                appshow.flow_ang_x = flow_o.ang_x;
      //		appshow.flow_fix_x_i = flow_o.fix_x_i;
      //                appshow.flow_ang_y = flow_o.ang_y;
      //		appshow.flow_fix_y_i = flow_o.fix_y_i;
      //                
      //                
                      appshow.loc_x = flow_o.loc_x;
                      appshow.loc_y = flow_o.loc_y;
                      appshow.loc_xs = flow_o.loc_xs;
                      appshow.loc_ys = flow_o.loc_ys;
                      
                      
                      
                      
                      
                      appshow.delta_x = flow_o.out_x_i;
                      appshow.delta_y = flow_o.out_y_i;
                      
                      
                      appshow.motorA_out = motor.A.throttle;
                      appshow.motorB_out = motor.B.throttle;
                      appshow.motorC_out = motor.C.throttle;
                      appshow.motorD_out = motor.D.throttle;
                      
                      
                      
                      appshow.debug_date1= x_dot;
                      appshow.debug_date2= y_dot;
                      appshow.debug_date3= speedX;
                      appshow.debug_date4= speedY;
                      appshow.debug_date5= posX;
                      appshow.debug_date6= posY;
                      appshow.debug_date7= controlX;
                      appshow.debug_date8= controlY;
                      
                      appshow.setpitch = userset_pitch;
                      appshow.setroll  = userset_roll;
...
//		vTaskDelayUntil(&preTime,pdMS_TO_TICKS( (1.0f/(float)TaskFreq)*1000) );
                vTaskDelayUntil(&preTime,MAIN_LOOP_DT );
	}
}
...
void desired_accel_transform_angle(vector2f _accel_target,vector2f *target_angle,float cos_rol)
{
  ...
  target_angle->y=RAD_TO_DEG*atanf( accel_forward*cos_rol/(GRAVITY_MSS*100));//计算期望俯仰角
...
//	if( 0 == m->D.Telemetry )
		m->D.throttle = target_limit_s16(m->D.throttle,Dshot_MIN,Dshot_MAX);
         ...
         m_out.A.throttle = m->A.throttle/2047.0f*4000+4000;
         m_out.B.throttle = m->B.throttle/2047.0f*4000+4000;
         m_out.C.throttle = m->C.throttle/2047.0f*4000+4000;
         m_out.D.throttle = m->D.throttle/2047.0f*4000+4000;
         
	//发送油门指令
	pMOTORInterface_t motor = &UserMotor;
	motor->setduty(m_out.C.throttle,m_out.B.throttle,m_out.A.throttle,m_out.D.throttle);
			...
			if( state==6 )  
			{
				...
				if( stablecount == calibratetimes ) 
				{
					...
					res_p.axis = &ZeroPoint;
                                           ...
                                           xEventGroupSetBits(g_xEventFlyAction,IMU_axilValZeroDone_Event); //设置事件,IMU标定完成
