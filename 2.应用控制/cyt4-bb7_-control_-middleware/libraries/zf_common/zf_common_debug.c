...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     debug 软延时函数 在 250MHz 下是一秒左右的时间
// 各单片机需要根据各自时钟试验 参数说明     void 返回参数     void 使用示例
// debug_delay(); 备注信息     本函数在文件内部调用 用户不用关注 也不可修改
//-------------------------------------------------------------------------------------------------------------------
...
static void debug_output(char *type, char *file, int line, char *str) {
  ...
  if (zf_debug_init_flag) {
    if (debug_output_info.type_index) {
      ...
      len_origin = strlen(file);
      show_len =
          (debug_output_info.display_x_max / debug_output_info.font_x_size);

      while (*file_str++ != '\0')
        ;

      // 只取一级目录 如果文件放在盘符根目录 或者 MDK 的工程根目录
      // 就会直接输出当前目录
      ...
      for (i = 0; i < ((strlen(output_buffer) / show_len) + 1); i++) {
        ...
        debug_output_info.output_screen(
            0, debug_output_info.font_y_size * show_line_index++,
            file_path_buffer);
      }
      ...
      debug_output_info.output_screen(
          0, debug_output_info.font_y_size * show_line_index++, output_buffer);
...
void debug_interrupr_handler(void) {
  ...
  if (zf_debug_init_flag) {
    ...
    // 3. 有数据才读取，超时会退出
    rec_sta = uart_read_self_quit(DEBUG_UART_INDEX, &debug_uart_data, 1000);
    if (rec_sta) {
      // 如果接收成功
...
// 备注信息     断言默认开启 建议开启断言
//-------------------------------------------------------------------------------------------------------------------
void debug_assert_enable(void) { zf_debug_assert_enable = 1; }
...
// 备注信息     断言默认开启 不建议禁用断言
//-------------------------------------------------------------------------------------------------------------------
void debug_assert_disable(void) { zf_debug_assert_enable = 0; }
...
void debug_init(void) {
  debug_info_init(); // debug 串口信息初始化
   ...
   uart_init(DEBUG_UART_INDEX,    // 在 zf_common_debug.h 中查看对应值
             ...
             DEBUG_UART_RX_PIN,8,CY_SCB_UART_PARITY_NONE,CY_SCB_UART_STOP_BITS_1);  // 在 zf_common_debug.h 中查看对应值
#if DEBUG_UART_USE_INTERRUPT // 条件编译 只有在启用串口中断才编译
