...
//					xEventGroupSetBits(g_xEventFlyAction,StartFly_Event);
                ...
                static uint8_t flog_start = 0;
		/* 标定零点程序 END */
		if( syscount > TaskFreq*20 && flog_start== 0)
                {
                  flog_start=1;
                  xEventGroupSetBits(g_xEventFlyAction,StartFly_Event);
                }
...
////			// 坐标变换(机体坐标系 --> 世界坐标系)
			 a_motion_x = a_x * cos(AttitudeVal.pitch) + 
					   a_y * sin(AttitudeVal.roll) * sin(AttitudeVal.pitch)-
							 a_z * cos(AttitudeVal.roll) * sin(AttitudeVal.pitch);
			 a_motion_y = a_y * cos(AttitudeVal.roll) +
						   a_z * sin(AttitudeVal.roll);
//                         a_motion_y = a_y * cos(AttitudeVal.roll) + 
//                                               a_x * sin(AttitudeVal.pitch) * sin(AttitudeVal.roll) + 
//                                                             a_z * cos(AttitudeVal.pitch) * sin(AttitudeVal.roll);
                        
			if( fabs(a_motion_x)<0.15f ) a_motion_x=0;
			if( fabs(a_motion_y)<0.15f ) a_motion_y=0;
                        
                        
                        a_motion_x += (a_motion_x - a_motion_xlast) *0.15;
                        a_motion_y += (a_motion_y - a_motion_ylast) *0.15;
                        a_motion_xlast = a_motion_x;a_motion_ylast = a_motion_y;
//                        a_motion_y+=0.01;
                        
////			
////			// 互补滤波
			x_dot = 0.85f * (x_dot + a_motion_x * 0.005f) +  0.15f * speedX;// 单位：m/s
			y_dot = 0.85f * (y_dot + a_motion_y * 0.005f) +  0.15f * speedY;// 单位：m/s
...
//	if( 0 == m->D.Telemetry )
		m->D.throttle = target_limit_s16(m->D.throttle,Dshot_MIN,Dshot_MAX);
         ...
         m_out.A.throttle = m->A.throttle/2047.0f*1000+1000;
         m_out.B.throttle = m->B.throttle/2047.0f*1000+1000;
         m_out.C.throttle = m->C.throttle/2047.0f*1000+1000;
         m_out.D.throttle = m->D.throttle/2047.0f*1000+1000;
